...
task :all_routes_pro => :environment do
  CSV.foreach('data/historicos.csv', :headers => true, :col_sep => ';') do |row|
    ...
    aux_routes = Route.by_start
    exist = false
    aux_routes.each do |ar|
      aux_route_clients = ar.route_clients
      aux_route_clients.each do |arc|
        if arc.client_id == client.id
          exist = true
          arc.orders << order
          break
        end
